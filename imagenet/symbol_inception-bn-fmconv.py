...
def ConvFactory(data, num_filter, kernel, stride=(1,1), pad=(0, 0), name=None, suffix='', act_type='relu'):
    ...
    act = mx.symbol.Activation(data=bn, act_type=act_type, name='%s_%s%s' %(act_type, name, suffix))
    return act

def InceptionFactoryA(data, num_1x1, num_3x3red, num_3x3, num_d3x3red, num_d3x3, pool, proj, name, act_type='relu'):
    # 1x1
    c1x1 = ConvFactory(data=data, num_filter=num_1x1, kernel=(1, 1), name=('%s_1x1' % name), act_type=act_type)
    # 3x3 reduce + 3x3
    c3x3r = ConvFactory(data=data, num_filter=num_3x3red, kernel=(1, 1), name=('%s_3x3' % name), suffix='_reduce')
    c3x3 = ConvFactory(data=c3x3r, num_filter=num_3x3, kernel=(3, 3), pad=(1, 1), name=('%s_3x3' % name), act_type=act_type)
    # double 3x3 reduce + double 3x3
    cd3x3r = ConvFactory(data=data, num_filter=num_d3x3red, kernel=(1, 1), name=('%s_double_3x3' % name), suffix='_reduce')
    cd3x3 = ConvFactory(data=cd3x3r, num_filter=num_d3x3, kernel=(3, 3), pad=(1, 1), name=('%s_double_3x3_0' % name))
    cd3x3 = ConvFactory(data=cd3x3, num_filter=num_d3x3, kernel=(3, 3), pad=(1, 1), name=('%s_double_3x3_1' % name), act_type=act_type)
    # pool + proj
    pooling = mx.symbol.Pooling(data=data, kernel=(3, 3), stride=(1, 1), pad=(1, 1), pool_type=pool, name=('%s_pool_%s_pool' % (pool, name)))
    cproj = ConvFactory(data=pooling, num_filter=proj, kernel=(1, 1), name=('%s_proj' %  name), act_type=act_type)
    # concat
...
def get_symbol(num_classes=1000):
    ...
    in5b = InceptionFactoryA(in5a, 352, 192, 320, 192, 224, "max", 128, '5b', act_type='tanh')
    
    bilinear = mx.symbol.FMConvolution1(data=in5b, num_filter=num_classes, num_factor=20,
                                        kernel=(1, 1), stride=(1, 1),
                                        pad=(0, 0), p=0.5, name='bilinear1')
    conv = mx.symbol.Convolution(data=in5b, num_filter=num_classes,
                                 kernel=(1, 1), stride=(1, 1),
                                 pad=(0, 0), name='fc')
    pool = mx.symbol.Pooling(
        data=conv + bilinear, pool_type="avg", kernel=(7, 7), name="global_pool")
